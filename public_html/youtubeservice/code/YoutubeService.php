...
class YoutubeService extends RestfulService {
    ...
    function errorCatch($response) {
        ...
        if (strpos($err_msg, '<') === false)
            user_error("YouTubeService Error : $err_msg", E_USER_ERROR);
    ...
    function extractVideoInfo($video) {
        ...
        $urlObj = $mediaentry->xpath('media:player');
        $data['PlayerID'] = $this->getYoutubeID(Convert::raw2xml((string) $urlObj[0]['url']));
        return $data;
    }
